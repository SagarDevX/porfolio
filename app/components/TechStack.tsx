...
const TechStack = () => {
  return (
    <div>
      ...
      {Tech.map((item)=>(
        <div  className="rounded-md font-semibold transition-colors duration-200 bg-neutral-200 text-primary hover:bg-neutral-300 inline-flex items-center gap-1.5 border border-border/50 px-3 py-1.5 text-sm">
          <item.icon size={20}/>
          <h1>{item.name}</h1>
        </div>
      ))}
